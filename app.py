...
def build_model(df):
    ...
    with st.markdown('**| Adjusted R-Squared **'):
        ...
        ax1.set(xlim=(0, 1))        
    st.markdown(imagedownload(plt,'plot-ar2-tall.pdf'), unsafe_allow_html=True)
        # Wide
    plt.figure(figsize=(9, 3))
    sns.set_theme(style="whitegrid")
    ax1 = sns.barplot(x=predictions_test.index, y="Adjusted R-Squared", data=predictions_test)
    ax1.set(ylim=(0, 1))
    plt.xticks(rotation=90)
    st.pyplot(plt)
    st.markdown(imagedownload(plt,'plot-ar2-wide.pdf'), unsafe_allow_html=True)
...
if uploaded_file is not None:
    ...
    st.write(df)
    build_model(df)
elif uploaded_file_xlsx is not None:
